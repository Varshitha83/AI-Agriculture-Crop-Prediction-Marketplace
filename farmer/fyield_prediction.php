...
// Load JSON files
Promise.all([
  fetch('state_districts.json').then(r => r.json()).then(data => { stateDistrictData = data; }),
  fetch('crop_seasons.json').then(r => r.json()).then(data => { cropSeasonData = data; })
])
.then(() => {
  console.log('Data loaded successfully');
  console.log('States:', Object.keys(stateDistrictData).length);
  console.log('Seasons:', Object.keys(cropSeasonData).length);
  populateStates();
  populateSeasons();
})
...
function populateStates() {
  ...
  console.log('Populating states:', states.length);
...
function populateSeasons() {
  ...
  const seasons = Object.keys(cropSeasonData).sort();
  console.log('Populating seasons:', seasons.length);
...
// Handle season change - populate crops
document.getElementById('season').addEventListener('change', function() {  
  ...
  if (selectedSeason && cropSeasonData[selectedSeason]) {
    const crops = cropSeasonData[selectedSeason];
    console.log('Crops for ' + selectedSeason + ':', crops.length);
...
<div class="card text-white bg-gradient-success mb-3">
				  ...
				  <span class=" text-success display-4" > Result  </span>					
					...
					$command = escapeshellcmd("C:\\Users\\DELL\\AppData\\Local\\Programs\\Python\\Python313\\python.exe ML/yield_prediction/yield_prediction.py $Jstate $Jdistrict $Jseason $Jcrops $Jarea ");
                    ob_start();
                    passthru($command);
                    $output = ob_get_clean();
                    echo '<div style="font-size: 18px; color: #27ae60; font-weight: bold;">' . htmlspecialchars($output) . '</div>';
